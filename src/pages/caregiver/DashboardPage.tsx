...
export function CaregiverDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-7">
      ...
      <p className="mt-1 text-[12.5px] text-ink/50">{caregiver?.name}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-[14px] border border-linha bg-linha sm:grid-cols-3">
        {[
          ...
          <div key={item.label} className="bg-surface-raised px-3.5 py-3 last:col-span-2 sm:last:col-span-1">
            <div className="text-[10px] tracking-wide text-ink/40 uppercase">{item.label}</div>
            <div className="mt-1 flex items-center gap-1 font-mono text-[17px] leading-none">
      ...
      <h2 className="mt-7 mb-2.5 text-[11px] font-semibold tracking-wide text-ink/40 uppercase">
        Próximo atendimento
      </h2>
      {!next ? (
        ...
        <Link to={`/cuidador/atendimentos/${next.id}`} className="block">
          <Card className="transition-colors duration-200 ease-out hover:border-accent">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[13.5px] font-semibold">{patientName(next.patientId)}</div>
            ...
            <div className="mt-2 flex flex-wrap gap-x-4 text-[11.5px] text-ink/60">
        ...
        <div className="flex flex-col gap-1.5">
          {upcoming.slice(1, 5).map((a) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate text-[13px]">{patientName(a.patientId)}</span>
              <span className="font-mono text-[11.5px] text-ink/50">{a.startTime}</span>
            </Link>
          ))}
        </div>
      )}
      ...
      {invitations.length > 0 && (
        <Link
          ...
          className="mt-6 inline-flex items-center gap-2 rounded-[10px] bg-accent px-4 py-2.5 text-[13.5px] font-semibold text-accent-ink transition-transform duration-[80ms] active:scale-[0.96]"
        >
